...
export default function UnregisteredCustomersPage() {
  ...
  const { data: customers, isLoading } = useQuery({
    queryKey: ['unregistered-customers', searchParams],
    queryFn: async () => {
      // home 페이지의 미등록 고객 데이터를 사용
      const stored = localStorage.getItem('unregisteredCustomers');
      if (stored) {
  ...
  const handleUnregisteredSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    
    // 미등록 고객 localStorage에 추가
    const existingUnregisteredCustomers = JSON.parse(localStorage.getItem('unregisteredCustomers') || '[]')
    const newUnregisteredCustomer = {
      ...form,
      id: crypto.randomUUID(),
    }
    localStorage.setItem('unregisteredCustomers', JSON.stringify([...existingUnregisteredCustomers, newUnregisteredCustomer]))
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-800">미등록 고객 관리</h1>
      </div>

      {/* 날짜 네비게이션 + 검색 + 미등록고객추가 버튼 한 줄 배치 */}
      <div className="flex items-center justify-between mb-4 gap-4">
        ...
        <Dialog open={showUnregisteredModal} onOpenChange={setShowUnregisteredModal}>
          <DialogTrigger asChild>
            <Button onClick={() => setShowUnregisteredModal(true)} className="bg-blue-600 hover:bg-blue-700">
              미등록 고객 추가
            </Button>
          </DialogTrigger>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>미등록 고객 등록</DialogTitle>
            </DialogHeader>
            <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={handleUnregisteredSubmit}>
              ...
              <div className="md:col-span-2 flex gap-2 justify-end mt-2">
                <Button type="submit" className="bg-blue-600 hover:bg-blue-700">미등록 고객 등록</Button>
                <DialogClose asChild>
