  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">신규 등록 고객 관리</h2>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button onClick={() => { setForm(initialForm); setEditingId(null); }}>신규 등록 고객 추가</Button>
          </DialogTrigger>
          <DialogContent className="max-w-3xl">
            <DialogHeader>
              <DialogTitle>{editingId ? '신규 등록 고객 수정' : '신규 등록 고객 등록'}</DialogTitle>
            </DialogHeader>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white rounded shadow">
          ...
          <tbody>
            ...
            {customers.length === 0 && (
              <tr>
                <td colSpan={10} className="text-center p-4 text-gray-400">등록된 신규 등록 고객이 없습니다.</td>
              </tr>
            )}
          </tbody>
        </table>
